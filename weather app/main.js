...
const lastsearch = [];
const todaytem = document.querySelector(".temp");
...
async function getData(cityName) {
  ...
  let response = await fetch(
    // `https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&units=metric&cnt=16&appid=1cfec3f331a083f1d208363ad639dcff`
    `https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&appid=1cfec3f331a083f1d208363ad639dcff`
  );
  if (response.ok) {
...
function changeImg(element, value) {
  if (value == "Clear") {
    ...
    element.src = `images/LightRain.png`;
  } else if ((value = "Light Cloud")) {
    element.src = `images/LightCloud.png`;
  } else if (value == "Shower") {
    element.src = `images/Shower.png`;
  } else if (value == "Heavy Cloud") {
    element.src = `images/HeavyCloud.png`;
  } else {
    element.src = `images/Thunderstorm.png`;
  }
}
...
function searchDataFunc(city) {
  ...
  getData(cityName)
    .then((data) => {
      ...
      windStatus.innerHTML = `${data.list[0].wind.speed} Mph`;
